            ...
            <div class="container-fluid my-4">
                <div class="d-md-flex align-items-center justify-content-between">
                    ...
                    <div class="text-end my-2 mt-md-0">
                        <a class="btn btn-outline-primary" href="{{ route('transfer.index') }}">
                            Back
                        </a>
                    </div>
                </div>
                <div class="card">
                    <div class="card-body">
                        <form action="{{ route('transfer.update', $editTransferData->id) }}" method="post"
                            ...
                            <div class="row">
                                ...
                                <input type="hidden" name="from_warehouse_id"
                                    value="{{ $editTransferData->from_warehouse_id }}">

                                <input type="hidden" name="to_warehouse_id"
                                    value="{{ $editTransferData->to_warehouse_id }}">
                                        ...
                                        <select name="from_warehouse_id" id="warehouse_id" class="form-control form-select"
                                            ...
                                            @foreach ($warehouses as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ $editTransferData->from_warehouse_id == $item->id ? 'selected' : '' }}>
                                                    {{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-4 mb-3">
                                    <div class="form-group w-100">
                                        ...
                                        <select name="to_warehouse_id" id="warehouse_id" class="form-control form-select"
                                            ...
                                            @foreach ($warehouses as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ $editTransferData->to_warehouse_id == $item->id ? 'selected' : '' }}>
                                                    {{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                        <small id="warehouse_error" class="text-danger d-none">Please select the
                                ...
                                <div class="col-md-12">
                                    ...
                                    <table class="table table-striped table-bordered dataTable" style="width: 100%;">
                                        ...
                                        <tbody id="productBody">
                                            @foreach ($editTransferData->transferItems as $item)
                                                <tr data-id={{ $item->id }}>

                                                    <input type="hidden" name="products[{{ $item->product->id }}][transfer_item_id]"
                                                        value="{{ $item->id }}" style="max-width: 90px;">
                                                    
                                                    <input type="hidden" name="products[{{ $item->product->id }}][product_id]"
                                                        value="{{ $item->product->id }}" style="max-width: 90px;">
                                                    ...
                                                    <td>
                                                        ...
                                                        <input type="number"
                                                            ...
                                                            value="{{ $item->net_unit_cost }}" style="max-width: 90px;"
                                                            readonly>
                                    ...
                                    <div class="form-group w-100">
                                        ...
                                        <select name="status" id="status" class="form-control form-select">
                                            ...
                                            <option value="Transfer"
                                                {{ $editTransferData->status == 'Transfer' ? 'selected' : '' }}>
                                                Transfer</option>
                                            <option value="Pending"
                                                {{ $editTransferData->status == 'Pending' ? 'selected' : '' }}>
                                                Pending</option>
                                            <option value="Ordered"
                                                {{ $editTransferData->status == 'Ordered' ? 'selected' : '' }}>
                                                Ordered</option>
                                        </select>
                                        @error('status')
                                            <span class="text-danger">{{ $message }}</span>
                            ...
                            <div class="col-md-12 mt-2">
                                ...
                                <textarea class="form-control" name="note" rows="3" placeholder="Enter Notes">{{ $editTransferData->note }}</textarea>
                            </div>
